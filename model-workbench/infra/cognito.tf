# The Cognito gate is the plank's first cost guardrail: every model route
# requires a valid JWT, and — like planks 6 and 7 — there is NO self-signup
# and the demo credential is never printed on the site. Admin-created users only.
...
data "aws_ssm_parameter" "demo_password" {
  ...
  name  = "/boardwalk/model-workbench/demo-password"
}
