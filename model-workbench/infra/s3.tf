# The static workbench site. Unlike planks 8/9/11 there is no teardown half —
# Bedrock bills per token ($0 idle), so this plank is always-on and the cost
# guardrails live in the Cognito gate + DynamoDB daily caps instead.
