...
resource "aws_cloudfront_response_headers_policy" "security" {
  ...
  security_headers_config {
    ...
    content_security_policy {
      # Everything is same-origin: no Cognito, no direct-to-S3 uploads — the
      # page only ever talks to /api/* on its own hostname.
...
resource "aws_cloudfront_distribution" "site" {
  ...
  comment             = "Alpenglow Service Dispatch (aws-boardwalk / event-mesh)"
