...
resource "aws_cloudfront_response_headers_policy" "security" {
  ...
  security_headers_config {
    ...
    content_security_policy {
      # Everything is same-origin: the page only talks to /api/* on its own
      # hostname, and report artifacts are plain HTML with inline styles
      # (style-src 'unsafe-inline' covers them) served from the same bucket.
...
resource "aws_cloudfront_distribution" "site" {
  ...
  comment             = "Alpenglow Batch Works (aws-boardwalk / container-works)"
  ...
  # Report artifacts are written by the container and expire after 48h — never
  # cache them long enough to outlive the S3 object.
  ordered_cache_behavior {
