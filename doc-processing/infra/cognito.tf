# The Cognito gate is the plank's first cost guardrail: uploading a document
# (the only operation that spends Textract/Comprehend/Bedrock money) requires
# a valid JWT. Like plank 6 — and unlike plank 1 — there is NO self-signup and
# the demo credential is never printed on the site. Admin-created users only.
...
data "aws_ssm_parameter" "demo_password" {
  ...
  name  = "/boardwalk/doc-processing/demo-password"
}
