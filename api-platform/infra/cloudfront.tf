locals {
  ...
  api_origin_id = "rest-api"
  # Regional REST API endpoint; CloudFront adds the stage via origin_path.
  api_domain = "${aws_api_gateway_rest_api.api.id}.execute-api.${local.region}.amazonaws.com"
  ...
  origin_all_viewer_ehh = "b689b0a8-53d0-40ab-baf2-68738e2966ac" # AllViewerExceptHostHeader (forwards x-api-key)
}
...
resource "aws_cloudfront_response_headers_policy" "security" {
  ...
  security_headers_config {
    ...
    content_security_policy {
      # Everything is same-origin: the docs page only ever fetches its own
      # /openapi.json, /config.json, and the API under /v1/* and /v2/*.
...
resource "aws_cloudfront_distribution" "site" {
  ...
  comment             = "City of Alpenglow Developer API (aws-boardwalk / api-platform)"
  ...
  # Same-origin API under the real versioned paths: api.demos.planetek.org/v1/*
  # and /v2/* — no CORS anywhere, and the version prefix reads like a proper
  # public API rather than a proxied one. Host header must NOT be forwarded or
  # API Gateway can't route; x-api-key IS forwarded (AllViewerExceptHostHeader).
