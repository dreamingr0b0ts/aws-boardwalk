...
resource "aws_cloudfront_function" "spa_router" {
  ...
  publish = true
  code    = file("${path.module}/cf-spa-router.js")
}
...
resource "aws_cloudfront_distribution" "site" {
  ...
  comment             = "Alpenglow Permits (aws-boardwalk / modern-web-app)"
  ...
  # Same-origin API: the SPA calls /api/* on its own hostname — no CORS
