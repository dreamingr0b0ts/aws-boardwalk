...
- **Cost guardrails**: free-tier-first; banned always-on: NAT, ALB, RDS/OpenSearch instances, EKS, SageMaker endpoints, 24/7 Fargate. Budget tripwire at $35. **One deliberate paid exception (owner-approved 2026-07-20): the shared WAF web ACL** `platform-edge-acl` (platform root, ~$8/mo total) — CLOUDFRONT scope, attached to every distribution via data-source lookup; per-IP rate limit (300/5min) + Amazon IP reputation + Known Bad Inputs; NO Core Rule Set (would false-positive on plank 11/12's SQL-text and prompt bodies) and NO WAF logging (cost). New planks must attach it (copy any plank's `data "aws_wafv2_web_acl"` block).
