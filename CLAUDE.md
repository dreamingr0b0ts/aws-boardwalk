...
**Deploy-demo-teardown planks (8 Security, 9 Network, 11 Relational Registry — all live) use TWO Terraform roots:** an always-on `infra/` root (static site + persisted artifacts, in the CI matrix like any plank) and a `demo/` root holding every daily-billing resource, driven ONLY by local `make demo` / `make teardown` and **never added to the CI matrix** — CI applying it would silently re-enable daily billing on the next push. The CI scan job still lints the whole repo, so the demo root's Terraform must pass Checkov/Trivy too. Plank 8 (`security-posture/`) is the reference implementation: evidence report outlives the stack in the site bucket's `evidence/` prefix (excluded from `make publish` sync), status.json flips the page's live/torn-down banner. Planks 9 and 11 add a twist: their always-on roots ALSO carry public APIs (plank 11's query exhibits, plank 9's visitor-triggered inspection rounds) that discover the demo stack via SSM parameters the demo root writes and answer 503 honestly between windows. **Forgotten windows are swept automatically:** `.github/workflows/demo-sweep.yml` runs nightly at 09:00 UTC and tears down any demo root left up (each plank's own `make teardown`, so evidence snapshots and status flips still happen). It is deliberately destroy-only — it can never re-create billing — and `workflow_dispatch` doubles as a remote kill switch. Its teardown powers required the `PlankInstanceProfiles` statement in the CI IAM fence (github-oidc.tf).
